...
def creating_finished_data():
    ...
    write_to_bin(os.path.join(unfinished_path, "train.src.shuf"), os.path.join(unfinished_path, "train.tgt.shuf"), train_bin_path, vocab_counter)
    write_to_bin(os.path.join(unfinished_path, "valid.src.shuf"), os.path.join(unfinished_path, "valid.tgt.shuf"), valid_bin_path)
    write_to_bin(os.path.join(unfinished_path, "test.src.shuf"), os.path.join(unfinished_path, "test.tgt.shuf"), test_bin_path)
...
def chunk_file(set_name, chunks_dir, bin_file):
    ...
    while not finished:
        if set_name == 'train':
            chunk_fname = os.path.join(chunks_dir, '%s_%04d.bin' % (set_name, chunk))
        else:
            chunk_fname = os.path.join(chunks_dir, '%s_%02d.bin' % (set_name, chunk))
...
if __name__ == "__main__":
    shuffle_text_data("train.src", "train.tgt", "train.src.shuf", "train.tgt.shuf")
    shuffle_text_data("valid.src", "valid.tgt", "valid.src.shuf", "valid.tgt.shuf")
    shuffle_text_data("test.src", "test.tgt", "test.src.shuf", "test.tgt.shuf")
